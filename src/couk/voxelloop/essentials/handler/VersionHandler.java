...
public class VersionHandler {
    ...
    private VersionHandler.UpdateResult result = VersionHandler.UpdateResult.SUCCESS;
     ...
     */
    public VersionHandler(Plugin plugin, int id, File file, UpdateType type, boolean announce) {
     ...
     */
    public VersionHandler(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback) {
     ...
     * @param announce True if the program should announce the progress of new updates in console.
     */
    public VersionHandler(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback, boolean announce) {
        this.plugin = plugin;
        this.type = type;
        ...
        try {
            this.url = new URL(VersionHandler.HOST + VersionHandler.QUERY + this.id);
     ...
     * @see UpdateResult
     */
    public VersionHandler.UpdateResult getResult() {
        this.waitForThread();
        return this.result;
    }
    ...
    private void downloadFile() {
        ...
        try {
            ...
            final byte[] data = new byte[VersionHandler.BYTE_SIZE];
            int count;
            if (this.announce) {
                this.plugin.getLogger().info("About to download a new update: " + this.versionName);
            }
            long downloaded = 0;
            while ((count = in.read(data, 0, VersionHandler.BYTE_SIZE)) != -1) {
                downloaded += count;
            ...
            this.result = VersionHandler.UpdateResult.FAIL_DOWNLOAD;
        } finally {
            try {
                if (in != null) {
                    in.close();
        ...
        try {
            ...
            while (e.hasMoreElements()) {
                ...
                if (!entry.isDirectory()) {
                    ...
                    int b;
                    final byte[] buffer = new byte[VersionHandler.BYTE_SIZE];
                    final FileOutputStream fos = new FileOutputStream(destinationFilePath);
                    final BufferedOutputStream bos = new BufferedOutputStream(fos, VersionHandler.BYTE_SIZE);
                    while ((b = bis.read(buffer, 0, VersionHandler.BYTE_SIZE)) != -1) {
        ...
        if (this.type != UpdateType.NO_VERSION_CHECK) {
            ...
            if (title.split(DELIMETER).length == 2) {
                ...
                if (this.hasTag(localVersion) || !this.shouldUpdate(localVersion, remoteVersion)) {
                    ...
                    this.result = VersionHandler.UpdateResult.NO_UPDATE;
                    return false;
                }
            } else {
                ...
                this.result = VersionHandler.UpdateResult.FAIL_NOVERSION;
                return false;
            }
        }
        return true;
    ...
    private boolean hasTag(String version) {
        for (final String string : VersionHandler.NO_UPDATE_TAG) {
    ...
    private boolean read() {
        try {
            ...
            conn.addRequestProperty("User-Agent", VersionHandler.USER_AGENT);
            ...
            this.versionName = (String) latestUpdate.get(VersionHandler.TITLE_VALUE);
            this.versionLink = (String) latestUpdate.get(VersionHandler.LINK_VALUE);
            this.versionType = (String) latestUpdate.get(VersionHandler.TYPE_VALUE);
            this.versionGameVersion = (String) latestUpdate.get(VersionHandler.VERSION_VALUE);
    ...
    public interface UpdateCallback {
         ...
         */
        void onFinish(VersionHandler updater);
    }
